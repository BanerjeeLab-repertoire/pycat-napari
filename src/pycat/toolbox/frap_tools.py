...
def estimate_bleach_depth(time: np.ndarray, bl_intensity: np.ndarray,
    ...
    try:
        ...
        return float(s(0.0))
    except Exception:  # broad-ok: scientific_result — degraded fallback to the first baseline sample when the spline cannot extrapolate to t=0 — a reasonable baseline estimate, not a fabricated fit result
        return float(bl_intensity[0])
...
def fit_frap_recovery(time: np.ndarray, norm_intensity: np.ndarray) -> dict:
    ...
    except Exception as e:  # broad-ok: scientific_result — a fit failure returns an all-NaN result AND warns the user — an honest missing value the caller can see, never a fabricated default
