"""Subcellular-object segmentation - the orchestrator, split out of segmentation_tools (1.6.243).

segment_subcellular_objects wires the families together per cell: FZ binarization -> puncta refinement
(the SNR/kurtosis/contrast gate) -> the absolute-intensity punctate-signal gate, honouring the punctate_gate
/ image_stats controls; run_ is the viewer entry point; compare_segmentation_speed A/B-times the fast vs
slow refiner via _segment_core. Moved VERBATIM - no threshold or operation-order change. With this move
segmentation_tools.py becomes a PURE re-export shim over the toolbox/segmentation/ package. Imports the
puncta / fz / intensity / morphology families it composes.
"""
