...
def _ask_copy_to_local(file_path, verdict):
    ...
    except Exception:  # broad-ok: optional_probe — Qt/layer-inspection best-effort — a UI probe that fails degrades gracefully
        return 'no'
    try:
...
def _copy_to_local_with_progress(file_path, verdict):
    ...
    try:
        ...
        except Exception as _rc_exc:  # broad-ok: write — best-effort storage/cache op — logged, the load continues
            from pycat.utils.general_utils import debug_log
            debug_log('dialogs: could not record cache copy origin', _rc_exc)
        return dst
    except Exception as e:  # broad-ok: write — best-effort storage/cache op — logged, the load continues
...
# ── the channel-naming dialog flow, moved out of file_io as a mixin (file_io_decomposition) ─────────
# `assign_channels_in_dialog` drives the `ChannelAssignmentDialog` above (co-located here), and its
# confidence gate `_channels_all_confident`. Verbatim; `FileIOClass` inherits it, so `self.…` call sites
# are unchanged. `derive_layer_name` deliberately stays in file_io (tests AST-parse it there) and is
# lazy-imported below to avoid the cycle.
class _DialogsMixin:
    ...
    def assign_channels_in_dialog(self, all_channels, is_mask=False, channel_info=None):
        ...
        """
        from pycat.file_io.file_io import derive_layer_name  # stays in file_io (tests AST-parse it there); lazy avoids a cycle
