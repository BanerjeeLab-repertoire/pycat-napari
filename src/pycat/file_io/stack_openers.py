...
class _StackOpenersMixin:
    ...
    def _open_stack_ims(self, file_path: str):
        ...
        except Exception as _e:  # broad-ok: optional_probe — format/metadata read logged via debug_log; open continues with a fallback
            ...
            for _pos, channel_idx in enumerate(channels_to_load):
                ...
                if n_t == 1 and n_z == 1:
                    ...
                    if channel_idx == 0 and pos_path == file_path:
                        ...
                        except (KeyError, OSError, Exception) as _probe_err:  # broad-ok: optional_probe — format-open failure surfaced to the user; the open aborts gracefully
                    ...
                    except Exception:  # broad-ok: optional_probe — best-effort probe → fallback value; a read failure must not crash the open
                        _plane0 = None
    ...
    def _open_stack_generic(self, file_path: str, ext: str):
        ...
        if reader_has_structure:
            ...
            except Exception as _mde:  # broad-ok: optional_probe — format/metadata read logged via debug_log; open continues with a fallback
    ...
    def _czi_open_metadata(self, file_path, image):
        ...
        try:
            ...
            except Exception as _pe:  # broad-ok: optional_probe — format/metadata read logged via debug_log; open continues with a fallback
    ...
    def _open_czi_streaming(self, file_path: str, image=None):
        ...
        try:
            ...
            return
        except Exception as _e:  # broad-ok: optional_probe — format-open failure surfaced to the user; the open aborts gracefully
